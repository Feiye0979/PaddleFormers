...
def mm_dpo_collate_fn(
    ...
    if not max_seq_len:
        max_seq_len = max(sum(len(sequence.token_ids) for sequence in sequences) for sequences in batch)
    real_lens = [sum(len(seq.token_ids) for seq in sequences) for sequences in batch]
    max_seq_len = calc_padding_size(max_seq_len, training_args)
    print(f"[SeqLen] real_lens={real_lens}, padded_to={max_seq_len}", flush=True)
...
def collate_fn(
    ...
    # === 打印真实上下文长度 ===
    real_lens = [sum(len(item.token_ids) for item in sequences) for sequences in batch]
    print(f"[SeqLen] real_lens={real_lens}, padded_to={max_seq_len}")
...
def mm_collate_fn(
    ...
    for batch_sequence in batch:
        ...
        for seq in batch_sequence:
            ...
            if get_rope_func is not None and _has_mm:
                ...
                # shape: (3, 1, seq_len) to match multimodal output format
                position_ids = np.stack([pos, pos, pos])[:, np.newaxis, :]
                original_position_ids.append(position_ids)

        if original_position_ids:
            # Stay in numpy: concat along last axis then pad with zeros
            if isinstance(original_position_ids[0], np.ndarray):
                concat_pos = np.concatenate(original_position_ids, axis=-1)  # (3, 1, seq_len)
                pad_len = max_seq_len - concat_pos.shape[2]
                if pad_len > 0:
                    padded_position_ids = np.pad(concat_pos, ((0, 0), (0, 0), (0, pad_len)))
                else:
                    padded_position_ids = concat_pos
            else:
                # paddle tensor path (multimodal with get_rope_func) - keep as-is, will be converted later
                original_position_ids = paddle.concat(original_position_ids, axis=-1)
                padded_position_ids = paddle.nn.functional.pad(
                    original_position_ids, pad=[0, max_seq_len - original_position_ids.shape[2]]
                )
    ...
    for key, tensors in zip(input_keys, transposed_list):
        # Filter out None and empty values using numpy only (no paddle ops in prefetch thread)
        filtered = [x for x in tensors if x is not None and (hasattr(x, '__len__') and len(x) > 0)]
        if not filtered:
            continue
        if key == "position_ids":
            # tensors are either numpy arrays or paddle Tensors (from get_rope_func path)
            # Convert paddle tensors to numpy to avoid GPU ops in prefetch thread
            np_list = []
            for x in filtered:
                if isinstance(x, paddle.Tensor):
                    np_list.append(x.numpy())
                else:
                    np_list.append(np.array(x))
            value = np.concatenate(np_list, axis=bs_idx_in_rope)
        else:
            np_list = []
            for x in filtered:
                if isinstance(x, paddle.Tensor):
                    ...
                    np_list.append(np.array(x) if not isinstance(x, np.ndarray) else x)
            try:
                value = np.concatenate(np_list, axis=0)
            except Exception:
                value = np_list[0]
