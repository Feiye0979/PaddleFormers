...
class Qwen3OmniMoeTextConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print("In ", __class__.__name__, ".__init__, ", type(self.rope_parameters), self.rope_parameters)
...
class Qwen3OmniMoeConfig(PretrainedConfig):
    ...
    def __setattr__(self, key, value):
        if (
            (thinker_config := super().__getattribute__("__dict__").get("thinker_config")) is not None
            and key not in ["_name_or_path", "model_type", "dtype", "_attn_implementation_internal"]
            and key in thinker_config.__dict__
        ):
            setattr(thinker_config, key, value)
        else:
            super().__setattr__(key, value)

    def __getattribute__(self, key):
        if "thinker_config" in super().__getattribute__("__dict__") and key not in [
            "_name_or_path",
            "model_type",
            "dtype",
            "_attn_implementation_internal",
        ]:
            thinker_config = super().__getattribute__("thinker_config")
            if key in thinker_config.__dict__:
                return getattr(thinker_config, key)
